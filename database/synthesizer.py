...
class DataSynthesizer:
    """Synthesizes and scales base HR dataset to 100,000+ rows with SCD Type 2 history."""

    def __init__(
        self,
        base_csv_path: str = "data/raw/WA_Fn-UseC_-HR-Employee-Attrition.csv",
        target_unique_employees: int = 65000,
    ):
        if not os.path.exists(base_csv_path):
            if os.path.exists("WA_Fn-UseC_-HR-Employee-Attrition.csv"):
                base_csv_path = "WA_Fn-UseC_-HR-Employee-Attrition.csv"
            else:
                raise FileNotFoundError(
                    f"Base CSV not found at: {base_csv_path}"
                )
    ...
    def run_pipeline(
        self,
        output_csv_path: str = "data/processed/synthesized_employee_data.csv",
    ) -> pd.DataFrame:
        """Executes full synthesis pipeline and saves dataset."""
